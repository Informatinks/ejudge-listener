...
def strip_cstring(cstring):
    """
    cstring is c style string with ending zero,
    return string withoout any extra characters in the end
    """
...
class EjudgeArchiveReader:
        ...
        archive_header = dict(
            zip(
                ...
                struct.unpack(
                    ...
                    file.read(
                        struct.calcsize(EjudgeArchiveReader.EJUDGE_ARCHIVE_HEADER_FMT)
                    ),
                ),
            )
        )
        del archive_header["padding"]
        archive_header["signature"] = strip_cstring(
            archive_header["signature"].decode('ascii')
        )
    ...
    def read_entry_header(file):
        ...
        # in the structure 4 + 4 + 4 + 4 bytes. There are not alignment bytes.
        # After the structure lie name of the file.
        read_entry_header["name"] = strip_cstring(
            file.read(
                read_entry_header["header_size"]
                - struct.calcsize(EjudgeArchiveReader.EJUDGE_ARCHIVE_ENTRY_HEADER_FMT)
            ).decode('ascii')
        )
    ...
    def __init__(self, path):
        ...
        if self.archive_header["version"] != self.VERSION:
            raise VersionError(
                "Ejudge Archive version is {0}, current supported version is {1}".format(
        ...
        while self.file.tell() < self.arch_size:
            ...
            self.files_positions[self.entry_headers_list[-1]["name"]] = (
                self.file.tell(),
                len(self.entry_headers_list) - 1,
            )
    ...
    def getfile(self, name):
        ...
        file_row = self.file.read(
            self.entry_headers_list[self.files_positions[name][1]]['size']
        )
