...
def normalizeMemoryLimit(memoryLimit):
    if memoryLimit[-1:] == "G":
        return int(memoryLimit[:-1]) * 1024 * 1024 * 1024
    if memoryLimit[-1:] == "M":
        return int(memoryLimit[:-1]) * 1024 * 1024
    if memoryLimit[-1:] == "K":
        return int(memoryLimit[:-1]) * 1024
...
class EjudgeProblemCfg:
    def __init__(self, d, id, contest):
        ...
        if "type" in d:
            ...
            if self.abstract != None and "type" in contest.abstract[self.abstract] and \
                    contest.abstract[self.abstract]["type"][0].strip("\"") == 'output-only':
                self.output_only = True
    ...
    def getInfo(self):
        return {"long_name": self.long_name, "id": self.id, "short_name": self.short_name, "timelimit": self.time_limit,
